...
def mlp(opts, input, output_dim, nlayers, init=None, stddev=0.0099999, bias=0., nonlinear='leaky_relu', alpha=1., scope=None, reuse=False):
    layer_x = input
    with tf.variable_scope(scope, reuse=reuse):
        for i in range(nlayers-1):
            layer_x = linear(opts, layer_x, np.prod(layer_x.get_shape().as_list()[1:]),
                        ...
                        reuse=reuse)
            layer_x = non_linear(layer_x, nonlinear, alpha)
    # output layer
